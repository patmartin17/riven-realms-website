...
import { Book, Sword, Castle, Map, Sparkles, Shield, Users, Scroll, HelpCircle, Play } from "lucide-react"

export const metadata = {
  title: "Wiki",
  description: "Learn everything about Riven Realms - guides, tutorials, and game mechanics.",
}
...
const quickLinks = [
  { name: "Server IP", value: "play.rivenrealms.com", icon: Play },
  { name: "Discord", value: "Join Community", icon: Users, href: "/discord" },
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="mb-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {quickLinks.map((link) => (
              <Card key={link.name} className="text-center py-6">
                <CardContent className="p-0">
                  <div className="flex justify-center mb-3">
                    <link.icon className="w-6 h-6 text-[#c77dff]" />
                  </div>
                  <div className="text-sm text-foreground/50 mb-1">{link.name}</div>
                  {link.href ? (
                    <Link href={link.href} className="font-cinzel font-bold text-foreground hover:text-[#c77dff] transition-colors">
                      {link.value}
                    </Link>
                  ) : (
                    <div className="font-mono text-sm text-[#c77dff]">{link.value}</div>
                  )}
                </CardContent>
              </Card>
            ))}
